...
def _make_dist_chart(df: pd.DataFrame, col: str) -> bytes | None:
    ...
    try:
        ...
        buf = io.BytesIO()
        plt.savefig(
            buf, format="png", dpi=120, bbox_inches="tight", facecolor="#1E293B"
        )
        plt.close(fig)
        buf.seek(0)
        return buf.read()
...
def _make_bar_chart(df: pd.DataFrame, cat_col: str, num_col: str) -> bytes | None:
    ...
    try:
        ...
        grouped = (
            df.groupby(cat_col)[num_col].sum().sort_values(ascending=False).head(8)
        )
        if len(grouped) == 0:
            return None
...
def generate_ppt_report(report_input: ReportInput) -> bytes:
    ...
    txt(s, "DATA ANALYSIS REPORT", 0.4, 0.3, 12, 0.6, size=11, bold=True, color=teal_l)
    title = (
        ri.file_name.replace(".csv", "").replace(".xlsx", "").replace("_", " ").title()
    )
    ...
    txt(
        s,
        f"Numeric: {len(ri.profile.numeric_columns)} cols",
        3.0,
        3.8,
        2.5,
        0.75,
    ...
    txt(
        s,
        f"Categorical: {len(ri.profile.categorical_columns)} cols",
        5.8,
        3.8,
        2.5,
        0.75,
    ...
    txt(s, f"  {branding}", 0, 6.85, 13.33, 0.5, size=11, color=slate, align="center")
    ...
    metrics = [
        ...
        (
            "Quality Score",
            f"{ri.profile.quality_score}/100",
    ...
    if ri.kpi:
        ...
        txt(s, "No numeric data available.", 0.4, 2.0, 12, 0.5, size=14, color=slate)
    txt(
        s,
        branding,
        0,
    ...
    txt(
        s,
        f"DISTRIBUTION  —  {ri.metric_col.upper()}",
        0.4,
        0.18,
        12,
        0.65,
    ...
    if ri.profile.has_categorical and ri.profile.has_numeric:
        ...
        txt(
            s,
            "Requires both numeric and categorical columns.",
            0.4,
            2.5,
            12,
            0.5,
    ...
    txt(
        s,
        "IQR method — flagged, not automatically removed",
        0.4,
        0.65,
        12,
        0.35,
    ...
    if ri.outliers:
        ...
        for header, xp in zip(headers, x_pos, strict=False):
            txt(s, header, xp, 1.1, 1.8, 0.42, size=10, bold=True, color=teal_l)
    ...
    if ri.ai_insights:
        ...
        txt(
            s,
            "AI insights not generated — add GROQ_API_KEY to enable.",
            0.4,
            2.0,
            12,
            0.5,
    ...
    txt(
        s,
        "Report generated by Excel Auto-Analyst",
        0.4,
        2.6,
        12,
        0.5,
    ...
    txt(
        s,
        "github.com/Shweta-Mishra-ai/excel-auto-analyst",
        0.4,
        6.5,
        12,
        0.4,
